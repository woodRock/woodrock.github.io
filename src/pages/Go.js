...
const GoBoard = () => {
  ...
  const calculateScore = () => {
    const newTerritory = Array(BOARD_SIZE)
      .fill()
      .map(() => Array(BOARD_SIZE).fill(null));
    ...
    const floodFill = (row, col) => {
      if (!isValidPosition(row, col) || visited[row][col])
        return { black: 0, white: 0, neutral: 0 };
      visited[row][col] = true;

      if (currentBoard[row][col] === "black")
        return { black: 1, white: 0, neutral: 0 };
      if (currentBoard[row][col] === "white")
        return { black: 0, white: 1, neutral: 0 };
  ...
  const parseSGF = (sgf) => {
    ...
    moves.forEach((move) => {
      ...
      const newBoard = newBoardHistory[newBoardHistory.length - 1].map(
        (row) => [...row],
      );
      newBoard[y][x] = color;
  ...
  const getCapturedStones = (board, row, col) => {
    ...
    directions.forEach(([dy, dx]) => {
      ...
      if (
        ...
        const group = [];
        if (
          !hasLiberty(board, newRow, newCol, opponentColor, new Set(), group)
        ) {
  ...
  return (
    <div className="go-container">
      ...
      <div className="controls">
        ...
        <button onClick={undoAll} disabled={currentStep === 0} title="Undo All">
          &#8676;
        </button>
        <button onClick={undo} disabled={currentStep === 0} title="Undo">
          &#8592;
        </button>
        <button
          onClick={redo}
          disabled={currentStep === boardHistory.length - 1}
          title="Redo"
        >
          &#8594;
        </button>
        ...
        <button onClick={exportSGF} title="Export SGF">
          &#8599;
        </button>
        <button onClick={() => fileInputRef.current.click()} title="Import SGF">
          &#8593;
        </button>
        <input
          type="file"
      ...
      <div className="go-board">
        <svg
          width={BOARD_SIZE * CELL_SIZE + LABEL_OFFSET * 2}
          height={BOARD_SIZE * CELL_SIZE + LABEL_OFFSET * 2}
        >
          {renderBoard()}
      ...
      <p className="current-player">
        Current player: <span className="player-name">{currentPlayer}</span>
      </p>
      <div>
