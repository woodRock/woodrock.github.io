...
import { fullscreenPdf, lastSonarPing } from "../utils/signals.ts";

export default function JuicedCard(
  { children, className = "" }: {
  ...
  const handleMouseMove = (e: MouseEvent) => {
    if (!cardRef.current || fullscreenPdf.value) return;
  ...
  useEffect(() => {
    ...
    const dist = Math.hypot(
      cardX - lastSonarPing.value.x,
      cardY - lastSonarPing.value.y,
    );
  ...
  return (
    <div
      ...
      style={{
        transform: fullscreenPdf.value
          ? "none"
          : `perspective(1000px) rotateX(${rotation.x}deg) rotateY(${rotation.y}deg)`,
        transformStyle: fullscreenPdf.value ? "flat" : "preserve-3d",
      }}
    >
      <div
        class={`relative h-full w-full rounded-3xl overflow-hidden ${
          pingPulse
            ? "ring-2 ring-indigo-500/50 shadow-[0_0_30px_rgba(99,102,241,0.4)]"
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              `radial-gradient(circle at ${glarePos.x}% ${glarePos.y}%, rgba(255, 255, 255, 0.15), transparent 60%), 
