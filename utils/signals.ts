...
export const classificationResult = signal<
  { species: string; confidence: number } | null
>(null);
...
export const lastSonarPing = signal<
  { x: number; y: number; timestamp: number } | null
>(null);
export const fullscreenPdf = signal<{ filename: string; title: string } | null>(
  null,
);
